...
# Modeling Notes: 
#   * Exploratory analyses revealed that positive mood ratings were positively skewed, thus a 
#   weakly informative lognormal prior was chosen for these positive mood scores
...
#   * POS is mainly an aggregate of anxious mood - a limitation addressed in Study 2 
...
study1.out <- '{posterior_save_dir}/EMA_S1_Bayesian_Posteriors/gaussian' %>% glue()
...
S1_POS_ucm <- brm_multiple(
    ...
    seed = 7201969, 
    save_all_pars = TRUE, 
    save_model = "S1_POS_ucm", 
    open_progress = FALSE,
    refresh = 0, 
    prior = prior_config
)

save(list = c("S1_POS_ucm", "S1_POS_ucm_form", "dat.study1_list", "dat.study1_model"), 
     file = '{study1.out}/S1_POS_ucm.RData' %>% glue()) 

sink(paste0(study1.model, '/S1_POS_ucm.txt'))
print(summary(S1_POS_ucm), digits = 5)
sink()

create_diagnostic_plots(S1_POS_ucm, dat.study1_list, n_samples = 100, dir_path = study1.graphics)
...
S1_POS_NegEvnt <- brm_multiple(
    ...
    seed = 7201969, 
    save_all_pars = TRUE, 
    save_model = "S1_POS_NegEvnt", 
    open_progress = FALSE,
    refresh = 0, 
    prior = prior_config
)

save(list = c("S1_POS_NegEvnt", "S1_POS_NegEvnt_form", "dat.study1_list", "dat.study1_model"), 
     file = '{study1.out}/S1_POS_NegEvnt.RData' %>% glue()) 

sink(paste0(study1.model, '/S1_POS_NegEvnt.txt'))
print(summary(S1_POS_NegEvnt), digits = 5)
sink()

create_diagnostic_plots(S1_POS_NegEvnt, dat.study1_list, n_samples = 100, dir_path = study1.graphics)
...
S1_POS_PosEvnt <- brm_multiple(
    ...
    seed = 7201969, 
    save_all_pars = TRUE, 
    save_model = "S1_POS_PosEvnt", 
    open_progress = FALSE,
    refresh = 0, 
    prior = prior_config
)

save(list = c("S1_POS_PosEvnt", "S1_POS_PosEvnt_form", "dat.study1_list", "dat.study1_model"), 
     file = '{study1.out}/S1_POS_PosEvnt.RData' %>% glue()) 

sink(paste0(study1.model, '/S1_POS_PosEvnt.txt'))
print(summary(S1_POS_PosEvnt), digits = 5)
sink()

create_diagnostic_plots(S1_POS_PosEvnt, dat.study1_list, n_samples = 100, dir_path = study1.graphics)
...
# Running model with priors (see above)
S1_POS_NegEvnt_DN <- brm_multiple(
    S1_POS_NegEvnt_DN_form,
    data = dat.study1_list, 
    chains = 3,
    cores = 3,
    iter = 15000,
    ...
    seed = 7201969, 
    save_all_pars = TRUE, 
    save_model = "S1_POS_NegEvnt_DN", 
    open_progress = FALSE,
    refresh = 0, 
    prior = prior_config
)

save(list = c("S1_POS_NegEvnt_DN", "S1_POS_NegEvnt_DN_form", "dat.study1_list", "dat.study1_model"), 
     file = '{study1.out}/S1_POS_NegEvnt_DN.RData' %>% glue()) 

sink(paste0(study1.model, '/S1_POS_NegEvnt_DN.txt'))
print(summary(S1_POS_NegEvnt_DN), digits = 5)
sink()

create_diagnostic_plots(S1_POS_NegEvnt_DN, dat.study1_list, n_samples = 100, dir_path = study1.graphics)
...
# Running model with priors (see above)
S1_POS_PosEvnt_DN <- brm_multiple(
    S1_POS_PosEvnt_DN_form,
    data = dat.study1_list, 
    chains = 3,
    cores = 3,
    iter = 15000,
    ...
    seed = 7201969, 
    save_all_pars = TRUE, 
    save_model = "S1_POS_PosEvnt_DN", 
    open_progress = FALSE,
    refresh = 0, 
    prior = prior_config
)

save(list = c("S1_POS_PosEvnt_DN", "S1_POS_PosEvnt_DN_form", "dat.study1_list", "dat.study1_model"), 
     file = '{study1.out}/S1_POS_PosEvnt_DN.RData' %>% glue()) 

sink(paste0(study1.model, '/S1_POS_PosEvnt_DN.txt'))
print(summary(S1_POS_PosEvnt_DN), digits = 5)
sink()

create_diagnostic_plots(S1_POS_PosEvnt_DN, dat.study1_list, n_samples = 100, dir_path = study1.graphics)
...
S1_POS_NegEvnt_Exp_form <- bf(
    POS ~ 1 + c.NegEvnt + prop_NegEvnt + (1 + c.NegEvnt|ID)
) + gaussian()

# Running model with priors (see above)
S1_POS_NegEvnt_Exp <- brm_multiple(
    S1_POS_NegEvnt_Exp_form,
    data = dat.study1_list, 
    chains = 3,
    cores = 3,
    iter = 15000,
    ...
    seed = 7201969, 
    save_all_pars = TRUE, 
    save_model = "S1_POS_NegEvnt_Exp", 
    open_progress = FALSE,
    refresh = 0, 
    prior = prior_config
)

save(list = c("S1_POS_NegEvnt_Exp", "S1_POS_NegEvnt_Exp_form", "dat.study1_list", "dat.study1_model"), 
     file = '{study1.out}/S1_POS_NegEvnt_Exp.RData' %>% glue()) 

sink(paste0(study1.model, '/S1_POS_NegEvnt_Exp.txt'))
print(summary(S1_POS_NegEvnt_Exp), digits = 5)
sink()

create_diagnostic_plots(S1_POS_NegEvnt_Exp, dat.study1_list, n_samples = 100, dir_path = study1.graphics)
...
S1_POS_PosEvnt_Exp_form <- bf(
    POS ~ 1 + c.PosEvnt + prop_PosEvnt + (1 + c.PosEvnt|ID)
) + gaussian()

# Running model with priors (see above)
S1_POS_PosEvnt_Exp <- brm_multiple(
    S1_POS_PosEvnt_Exp_form,
    data = dat.study1_list, 
    chains = 3,
    cores = 3,
    iter = 15000,
    ...
    seed = 7201969, 
    save_all_pars = TRUE, 
    save_model = "S1_POS_PosEvnt_Exp", 
    open_progress = FALSE,
    refresh = 0, 
    prior = prior_config
)

save(list = c("S1_POS_PosEvnt_Exp", "S1_POS_PosEvnt_Exp_form", "dat.study1_list", "dat.study1_model"), 
     file = '{study1.out}/S1_POS_PosEvnt_Exp.RData' %>% glue()) 

sink(paste0(study1.model, '/S1_POS_PosEvnt_Exp.txt'))
print(summary(S1_POS_PosEvnt_Exp), digits = 5)
sink()

create_diagnostic_plots(S1_POS_PosEvnt_Exp, dat.study1_list, n_samples = 100, dir_path = study1.graphics)
...
S1_POS_NegEvnt_DN_Exp_form <- bf(
    POS ~ 1 + c.NegEvnt + prop_NegEvnt + c.DN + (1 + c.NegEvnt|ID)
) + gaussian()

# Running model with priors (see above)
S1_POS_NegEvnt_DN_Exp <- brm_multiple(
    S1_POS_NegEvnt_DN_Exp_form,
    data = dat.study1_list, 
    chains = 3,
    cores = 3,
    iter = 15000,
    ...
    seed = 7201969, 
    save_all_pars = TRUE, 
    save_model = "S1_POS_NegEvnt_DN_Exp", 
    open_progress = FALSE,
    refresh = 0, 
    prior = prior_config
)

save(list = c("S1_POS_NegEvnt_DN_Exp", "S1_POS_NegEvnt_DN_Exp_form", "dat.study1_list", "dat.study1_model"), 
     file = '{study1.out}/S1_POS_NegEvnt_DN_Exp.RData' %>% glue()) 

sink(paste0(study1.model, '/S1_POS_NegEvnt_DN_Exp.txt'))
print(summary(S1_POS_NegEvnt_DN_Exp), digits = 5)
sink()

create_diagnostic_plots(S1_POS_NegEvnt_DN_Exp, dat.study1_list, n_samples = 100, dir_path = study1.graphics)
...
S1_POS_PosEvnt_DN_Exp_form <- bf(
    POS ~ 1 + c.PosEvnt + prop_PosEvnt + c.DN + (1 + c.PosEvnt|ID)
) + gaussian()

# Running model with priors (see above)
S1_POS_PosEvnt_DN_Exp <- brm_multiple(
    S1_POS_PosEvnt_DN_Exp_form,
    data = dat.study1_list, 
    chains = 3,
    cores = 3,
    iter = 15000,
    ...
    seed = 7201969, 
    save_all_pars = TRUE, 
    save_model = "S1_POS_PosEvnt_DN_Exp", 
    open_progress = FALSE,
    refresh = 0, 
    prior = prior_config
)

save(list = c("S1_POS_PosEvnt_DN_Exp", "S1_POS_PosEvnt_DN_Exp_form", "dat.study1_list", "dat.study1_model"), 
     file = '{study1.out}/S1_POS_PosEvnt_DN_Exp.RData' %>% glue()) 

sink(paste0(study1.model, '/S1_POS_PosEvnt_DN_Exp.txt'))
print(summary(S1_POS_PosEvnt_DN_Exp), digits = 5)
sink()

create_diagnostic_plots(S1_POS_PosEvnt_DN_Exp, dat.study1_list, n_samples = 100, dir_path = study1.graphics)
...
S1_POS_NegEvnt_Rct_form <- bf(
    POS ~ 1 + c.NegEvnt * c.DN + prop_NegEvnt + c.DN + (1 + c.NegEvnt|ID)
) + gaussian()

# Running model with priors (see above)
S1_POS_NegEvnt_Rct <- brm_multiple(
    S1_POS_NegEvnt_Rct_form,
    data = dat.study1_list, 
    chains = 3,
    cores = 3,
    iter = 15000,
    ...
    seed = 7201969, 
    save_all_pars = TRUE, 
    save_model = "S1_POS_NegEvnt_Rct", 
    open_progress = FALSE,
    refresh = 0, 
    prior = prior_config
)

save(list = c("S1_POS_NegEvnt_Rct", "S1_POS_NegEvnt_Rct_form", "dat.study1_list", "dat.study1_model"), 
     file = '{study1.out}/S1_POS_NegEvnt_Rct.RData' %>% glue()) 

sink(paste0(study1.model, '/S1_POS_NegEvnt_Rct.txt'))
print(summary(S1_POS_NegEvnt_Rct), digits = 5)
sink()

create_diagnostic_plots(S1_POS_NegEvnt_Rct, dat.study1_list, n_samples = 100, dir_path = study1.graphics)
...
S1_POS_PosEvnt_Rct_form <- bf(
    POS ~ 1 + c.PosEvnt * c.DN + prop_PosEvnt + c.DN + (1 + c.PosEvnt|ID)
) + gaussian()

# Running model with priors (see above)
S1_POS_PosEvnt_Rct <- brm_multiple(
    S1_POS_PosEvnt_Rct_form,
    data = dat.study1_list, 
    chains = 3,
    cores = 3,
    iter = 15000,
    ...
    seed = 7201969, 
    save_all_pars = TRUE, 
    save_model = "S1_POS_PosEvnt_Rct", 
    open_progress = FALSE,
    refresh = 0, 
    prior = prior_config
)

save(list = c("S1_POS_PosEvnt_Rct", "S1_POS_PosEvnt_Rct_form", "dat.study1_list", "dat.study1_model"), 
     file = '{study1.out}/S1_POS_PosEvnt_Rct.RData' %>% glue()) 

sink(paste0(study1.model, '/S1_POS_PosEvnt_Rct.txt'))
print(summary(S1_POS_PosEvnt_Rct), digits = 5)
sink()

create_diagnostic_plots(S1_POS_PosEvnt_Rct, dat.study1_list, n_samples = 100, dir_path = study1.graphics)
