...
NEG_NegEvnt_fixef <- as.data.frame(fixef(S2_NEG_NegEvnt_x_DN_prop.NegEvnt))
m_Evnt <- mean(dat.study2_list[[1]]$prop.NegEvnt)

DN_cat <- c("+1 SD DN", "-1 SD DN")
Evnt_cat <- c("Yes", "No")
Evnt_vals <- c(1 - m_Evnt, 0 - m_Evnt)

NEG_NegEvnt_int_df <- generate_reactivity_data(NEG_NegEvnt_fixef, intercept_name="Intercept", m_Evnt=m_Evnt, 
                                               Evnt_cat=Evnt_cat, Evnt_vals=Evnt_vals, Evnt_var="c.NegEvnt", 
                                               Evnt_m_var="prop.NegEvnt", Evnt_int_var="c.NegEvnt:c.DN", 
                                               DN_cat=DN_cat, DN_var="c.DN", log=TRUE, error_var = 0.00215)


w <- .75

g <- ggplot(NEG_NegEvnt_int_df[NEG_NegEvnt_int_df$Evnt_cat == "Yes", ], 
            aes(fill=fill))+
  ...
  geom_segment(data=NEG_NegEvnt_int_df[NEG_NegEvnt_int_df$Evnt_cat == "Yes" & NEG_NegEvnt_int_df$effect != "Mood Rating", ], 
               aes(x=xmin, xend=xmax, y=ymin, yend=ymin))+
  facet_wrap(.~DN_cat)+
  labs(fill="", x="", y="Negative Mood Rating", title = "Study 2 Decomposing DN x Negative Event Interaction: Negative Mood", 
...
png(filename = "~/dr-consulting_GH/shackman-umd-pax-ema-pub/plot_outputs/S2_NEG_NegEvnt_x_DN_waterfall.png", 
...
NEG_PosEvnt_fixef <- as.data.frame(fixef(S2_NEG_PosEvnt_x_DN_prop.PosEvnt))
m_Evnt <- mean(dat.study2_list[[1]]$prop.PosEvnt)

DN_cat <- c("+1 SD DN", "-1 SD DN")
Evnt_cat <- c("Yes", "No")
Evnt_vals <- c(1 - m_Evnt, 0 - m_Evnt)

NEG_PosEvnt_int_df <- generate_reactivity_data(NEG_PosEvnt_fixef, intercept_name="Intercept", m_Evnt=m_Evnt, 
                                               Evnt_cat=Evnt_cat, Evnt_vals=Evnt_vals, Evnt_var="c.PosEvnt", 
                                               Evnt_m_var="prop.PosEvnt", Evnt_int_var="c.PosEvnt:c.DN", 
                                               DN_cat=DN_cat, DN_var="c.DN", log=TRUE, error_var = 0.00215)
...
g <- ggplot(NEG_PosEvnt_int_df[NEG_PosEvnt_int_df$Evnt_cat == "Yes", ], 
  ...
  geom_segment(data=NEG_PosEvnt_int_df[NEG_PosEvnt_int_df$Evnt_cat == "Yes" & NEG_PosEvnt_int_df$effect != "Mood Rating", ], 
               aes(x=xmin, xend=xmax, y=ymin, yend=ymin))+
  facet_wrap(.~DN_cat)+
  labs(fill="", x="", y="Negative Mood Rating", title = "Study 2 Decomposing DN x Positive Event Interaction: Negative Mood", 
       caption = "DN = dispositional negativity")+
  theme_bw()+
  theme(axis.text.x = element_text(angle = 60, vjust=.5))
...
png(filename = "~/dr-consulting_GH/shackman-umd-pax-ema-pub/plot_outputs/S2_NEG_PosEvnt_x_DN_waterfall.png", 
    width=9, height=6, units="in", res=600)
g
dev.off()
