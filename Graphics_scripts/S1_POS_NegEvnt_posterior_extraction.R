...
LOAD_PATH <- "{GRAPHICS_DIR}/S1_POS_NegEvnt_river_base.RData" %>% glue()
EXECUTE_FLAG <- !file.exists(LOAD_PATH)

data_loader(POSTERIOR_DIR, "S1_POS_NegEvnt_Rct", DATA_FILEPATH, "S1_POS_ucm")
...
if(EXECUTE_FLAG) {
    ...
    #----------------------------------------------------------------------------------------------------------------------
    summary_filepath <- "{SUMMARY_DIR}/var_decomp_S1_POS_NegEvnt_x_DN.txt" %>% glue()
    within_vars <- c("c.NegEvnt", "c.NegEvnt:c.DN")
    between_vars <- c("c.DN", "prop_NegEvnt")
    random_vars <- c("c.NegEvnt")
    
    posterior_df <- posterior_samples_extractor(S1_POS_ucm, S1_POS_NegEvnt_Rct, obs_lvl_var="gaussian")
    ...
    data_loader(POSTERIOR_DIR, "S1_POS_NegEvnt_DN_Exp")
    summary_filepath <- "{SUMMARY_DIR}/var_decomp_S1_POS_NegEvnt_DN_Exp.txt" %>% glue()
    within_vars <- c("c.NegEvnt")
    between_vars <- c("c.DN", "prop_NegEvnt")
    random_vars <- c("c.NegEvnt")
    
    posterior_df <- posterior_samples_extractor(S1_POS_ucm, S1_POS_NegEvnt_DN_Exp, obs_lvl_var="gaussian")
    ...
    data_loader(POSTERIOR_DIR, "S1_POS_NegEvnt_DN")
    summary_filepath <- "{SUMMARY_DIR}/var_decomp_S1_POS_NegEvnt_DN.txt" %>% glue()
    within_vars <- c("c.NegEvnt")
    between_vars <- c("c.DN")
    random_vars <- c("c.NegEvnt")
    
    posterior_df <- posterior_samples_extractor(S1_POS_ucm, S1_POS_NegEvnt_DN, obs_lvl_var="gaussian")
    ...
    #----------------------------------------------------------------------------------------------------------------------
    # Next Set of inputs - lv2 no interaction
    data_loader(POSTERIOR_DIR, "S1_POS_NegEvnt_Exp")
    summary_filepath <- "{SUMMARY_DIR}/var_decomp_S1_POS_NegEvnt_Exp.txt" %>% glue()
    within_vars <- c("c.NegEvnt")
    between_vars <- c("prop_NegEvnt")
    random_vars <- c("c.NegEvnt")
    
    posterior_df <- posterior_samples_extractor(S1_POS_ucm, S1_POS_NegEvnt_Exp, obs_lvl_var="gaussian")
    ...
    data_loader(POSTERIOR_DIR, "S1_POS_NegEvnt")
    summary_filepath <- "{SUMMARY_DIR}/var_decomp_S1_POS_NegEvnt.txt" %>% glue()
    within_vars <- c("c.NegEvnt")
    between_vars <- NULL
    random_vars <- c("c.NegEvnt")
    
    posterior_df <- posterior_samples_extractor(S1_POS_ucm, S1_POS_NegEvnt, obs_lvl_var="gaussian")
...
riverplot_df_helper(model_variance_list, model_names, within_contrasts, between_contrasts, custom_contrasts, 
                    ...
                    main_filename = "{GRAPHICS_DIR}/S1_POS_NegEvnt_decomp.eps" %>% glue(), 
                    main_title = "Total Variance Decomposition: Positive Mood, DN, and Negative Events", 
                    custom_filename = "{GRAPHICS_DIR}/S1_POS_NegEvnt_DN_combined.eps" %>% glue(), 
                    custom_title = "DN Variance Decomposition: Positive Mood, DN, and Negative Events", 
                    combined_plot_filename = "{GRAPHICS_DIR}/S1_POS_NegEvnt_full_decomp.eps" %>% glue())
