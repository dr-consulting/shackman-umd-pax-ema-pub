...
LOAD_PATH <- "{GRAPHICS_DIR}/S2_NEG_PosEvnt_river_base.RData" %>% glue()
EXECUTE_FLAG <- !file.exists(LOAD_PATH)

data_loader(POSTERIOR_DIR, "S2_NEG_PosEvnt_x_DN_prop.PosEvnt", DATA_FILEPATH, "S2_NEG_ucm")
...
if(EXECUTE_FLAG){
    ...
    model_vars <- c('c.DN', 'c.PosEvnt', 'prop.PosEvnt', 'NEG')
    ...
    #----------------------------------------------------------------------------------------------------------------------
    summary_filepath <- "{SUMMARY_DIR}/var_decomp_S2_NEG_PosEvnt_x_DN.txt" %>% glue()
    within_vars <- c("c.PosEvnt", "c.PosEvnt:c.DN")
    between_vars <- c("c.DN", "prop.PosEvnt")
    random_vars <- c("c.PosEvnt")
    
    posterior_df <- posterior_samples_extractor(S2_NEG_ucm, S2_NEG_PosEvnt_x_DN_prop.PosEvnt, obs_lvl_var="gamma")
    ...
    data_loader(POSTERIOR_DIR, "S2_NEG_PosEvnt_DN_prop.PosEvnt")
    summary_filepath <- "{SUMMARY_DIR}/var_decomp_S2_NEG_PosEvnt_DN_Exp.txt" %>% glue()
    within_vars <- c("c.PosEvnt")
    between_vars <- c("c.DN", "prop.PosEvnt")
    random_vars <- c("c.PosEvnt")
    
    posterior_df <- posterior_samples_extractor(S2_NEG_ucm, S2_NEG_PosEvnt_DN_prop.PosEvnt, obs_lvl_var="gamma")
    ...
    data_loader(POSTERIOR_DIR, "S2_NEG_PosEvnt_DN")
    summary_filepath <- "{SUMMARY_DIR}/var_decomp_S2_NEG_PosEvnt_DN.txt" %>% glue()
    within_vars <- c("c.PosEvnt")
    between_vars <- c("c.DN")
    random_vars <- c("c.PosEvnt")
    
    posterior_df <- posterior_samples_extractor(S2_NEG_ucm, S2_NEG_PosEvnt_DN, obs_lvl_var="gamma")
    ...
    #----------------------------------------------------------------------------------------------------------------------
    # Next Set of inputs - lv2 no interaction
    data_loader(POSTERIOR_DIR, "S2_NEG_PosEvnt_prop.PosEvnt")
    summary_filepath <- "{SUMMARY_DIR}/var_decomp_S2_NEG_PosEvnt_Exp.txt" %>% glue()
    within_vars <- c("c.PosEvnt")
    between_vars <- c("prop.PosEvnt")
    random_vars <- c("c.PosEvnt")
    
    posterior_df <- posterior_samples_extractor(S2_NEG_ucm, S2_NEG_PosEvnt_prop.PosEvnt, obs_lvl_var="gamma")
    ...
    data_loader(POSTERIOR_DIR, "S2_NEG_PosEvnt")
    summary_filepath <- "{SUMMARY_DIR}/var_decomp_S2_NEG_PosEvnt.txt" %>% glue()
    within_vars <- c("c.PosEvnt")
    between_vars <- NULL
    random_vars <- c("c.PosEvnt")
    
    posterior_df <- posterior_samples_extractor(S2_NEG_ucm, S2_NEG_PosEvnt, obs_lvl_var="gamma")
...
riverplot_df_helper(model_variance_list, model_names, within_contrasts, between_contrasts, custom_contrasts, 
                    ...
                    main_filename = "{GRAPHICS_DIR}/S2_NEG_PosEvnt_decomp.eps" %>% glue(), 
                    main_title = "Total Variance Decomposition: Negative Mood, DN, and Positive Events", 
                    custom_filename = "{GRAPHICS_DIR}/S2_NEG_PosEvnt_DN_combined.eps" %>% glue(), 
                    custom_title = "DN Variance Decomposition: Negative Mood, DN, and Positive Events", 
                    combined_plot_filename = "{GRAPHICS_DIR}/S2_NEG_PosEvnt_full_decomp.eps" %>% glue())
