...
LOAD_PATH <- "{GRAPHICS_DIR}/S1_NEG_NegEvnt_river_base.RData" %>% glue()
EXECUTE_FLAG <- !file.exists(LOAD_PATH)

data_loader(POSTERIOR_DIR, "S1_NEG_NegEvnt_Rct", DATA_FILEPATH, "S1_NEG_ucm")
...
if(EXECUTE_FLAG) {
    ...
    #----------------------------------------------------------------------------------------------------------------------
    summary_filepath <- "{SUMMARY_DIR}/var_decomp_S1_NEG_NegEvnt_x_DN.txt" %>% glue()
    ...
    remove(list=c("S1_NEG_NegEvnt_Rct"))
    gc()
    
    #----------------------------------------------------------------------------------------------------------------------
    # Next Set of inputs - lv2 no interaction
    data_loader(POSTERIOR_DIR, "S1_NEG_NegEvnt_DN_Exp")
    summary_filepath <- "{SUMMARY_DIR}/var_decomp_S1_NEG_NegEvnt_DN_Exp.txt" %>% glue()
    ...
    # Just some memory saving...
    remove(list=c("S1_NEG_NegEvnt_DN_Exp"))
    gc()
    
    #----------------------------------------------------------------------------------------------------------------------
    # Next Set of inputs - only DN at level 2 of the equation
    data_loader(POSTERIOR_DIR, "S1_NEG_NegEvnt_DN")
    summary_filepath <- "{SUMMARY_DIR}/var_decomp_S1_NEG_NegEvnt_DN.txt" %>% glue()
    within_vars <- c("c.NegEvnt")
    between_vars <- c("c.DN")
    random_vars <- c("c.NegEvnt")
    ...
    # Just some memory saving...
    remove(list=c("S1_NEG_NegEvnt_DN"))
    gc()
    
    #----------------------------------------------------------------------------------------------------------------------
    # Next Set of inputs - lv2 no interaction
    data_loader(POSTERIOR_DIR, "S1_NEG_NegEvnt_Exp")
    summary_filepath <- "{SUMMARY_DIR}/var_decomp_S1_NEG_NegEvnt_Exp.txt" %>% glue()
    within_vars <- c("c.NegEvnt")
    ...
    # Just some memory saving...
    remove(list=c("S1_NEG_NegEvnt_Exp"))
    gc()
    #----------------------------------------------------------------------------------------------------------------------
    # Next Set of inputs - lv1 only - no level 2 predictors of any kind
    data_loader(POSTERIOR_DIR, "S1_NEG_NegEvnt")
    summary_filepath <- "{SUMMARY_DIR}/var_decomp_S1_NEG_NegEvnt.txt" %>% glue()
    within_vars <- c("c.NegEvnt")
    between_vars <- NULL
    random_vars <- c("c.NegEvnt")
...
riverplot_df_helper(model_variance_list, model_names, within_contrasts, between_contrasts, custom_contrasts, 
                    ...
                    main_filename = "{GRAPHICS_DIR}/S1_NEG_NegEvnt_decomp.eps" %>% glue(), 
                    main_title = "Total Variance Decomposition: Negative Mood, DN, and Negative Events", 
                    custom_filename = "{GRAPHICS_DIR}/S1_NEG_NegEvnt_DN_combined.eps" %>% glue(), 
                    custom_title = "DN Variance Decomposition: Negative Mood, DN, and Negative Events", 
                    combined_plot_filename = "{GRAPHICS_DIR}/S1_NEG_NegEvnt_full_decomp.eps" %>% glue())
