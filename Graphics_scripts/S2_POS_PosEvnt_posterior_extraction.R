...
LOAD_PATH <- "{GRAPHICS_DIR}/S2_POS_PosEvnt_river_base.RData" %>% glue()
EXECUTE_FLAG <- !file.exists(LOAD_PATH)

data_loader(POSTERIOR_DIR, "S2_POS_PosEvnt_Rct", DATA_FILEPATH, "S2_POS_ucm")
...
if(EXECUTE_FLAG) {
    ...
    model_vars <- c('c.DN', 'c.PosEvnt', 'prop.PosEvnt', 'POS')
    ...
    #----------------------------------------------------------------------------------------------------------------------
    summary_filepath <- "{SUMMARY_DIR}/var_decomp_S2_POS_PosEvnt_x_DN.txt" %>% glue()
    within_vars <- c("c.PosEvnt", "c.PosEvnt:c.DN")
    between_vars <- c("c.DN", "prop.PosEvnt")
    random_vars <- c("c.PosEvnt")
    
    posterior_df <- posterior_samples_extractor(S2_POS_ucm, S2_POS_PosEvnt_Rct, obs_lvl_var="gaussian")
    ...
    remove(list=c("S2_POS_PosEvnt_Rct"))
    gc()
    
    #----------------------------------------------------------------------------------------------------------------------
    # Next Set of inputs - lv2 no interaction
    data_loader(POSTERIOR_DIR, "S2_POS_PosEvnt_DN_Exp")
    summary_filepath <- "{SUMMARY_DIR}/var_decomp_S2_POS_PosEvnt_DN_Exp.txt" %>% glue()
    within_vars <- c("c.PosEvnt")
    between_vars <- c("c.DN", "prop.PosEvnt")
    random_vars <- c("c.PosEvnt")
    
    posterior_df <- posterior_samples_extractor(S2_POS_ucm, S2_POS_PosEvnt_DN_Exp, obs_lvl_var="gaussian")
    ...
    # Just some memory saving...
    remove(list=c("S2_POS_PosEvnt_DN_Exp"))
    gc()
    
    #----------------------------------------------------------------------------------------------------------------------
    # Next Set of inputs - only DN at level 2 of the equation
    data_loader(POSTERIOR_DIR, "S2_POS_PosEvnt_DN")
    summary_filepath <- "{SUMMARY_DIR}/var_decomp_S2_POS_PosEvnt_DN.txt" %>% glue()
    within_vars <- c("c.PosEvnt")
    between_vars <- c("c.DN")
    random_vars <- c("c.PosEvnt")
    
    posterior_df <- posterior_samples_extractor(S2_POS_ucm, S2_POS_PosEvnt_DN, obs_lvl_var="gaussian")
    ...
    #----------------------------------------------------------------------------------------------------------------------
    # Next Set of inputs - lv2 no interaction
    data_loader(POSTERIOR_DIR, "S2_POS_PosEvnt_Exp")
    summary_filepath <- "{SUMMARY_DIR}/var_decomp_S2_POS_PosEvnt_Exp.txt" %>% glue()
    within_vars <- c("c.PosEvnt")
    between_vars <- c("prop.PosEvnt")
    random_vars <- c("c.PosEvnt")
    
    posterior_df <- posterior_samples_extractor(S2_POS_ucm, S2_POS_PosEvnt_Exp, obs_lvl_var="gaussian")
    ...
    # Just some memory saving...
    remove(list=c("S2_POS_PosEvnt_Exp"))
    gc()
    #----------------------------------------------------------------------------------------------------------------------
    # Next Set of inputs - lv1 only - no level 2 predictors of any kind
    data_loader(POSTERIOR_DIR, "S2_POS_PosEvnt")
    summary_filepath <- "{SUMMARY_DIR}/var_decomp_S2_POS_PosEvnt.txt" %>% glue()
    within_vars <- c("c.PosEvnt")
    between_vars <- NULL
    random_vars <- c("c.PosEvnt")
    
    posterior_df <- posterior_samples_extractor(S2_POS_ucm, S2_POS_PosEvnt, obs_lvl_var="gaussian")
...
riverplot_df_helper(model_variance_list, model_names, within_contrasts, between_contrasts, custom_contrasts, 
                    ...
                    main_filename = "{GRAPHICS_DIR}/S2_POS_PosEvnt_decomp.eps" %>% glue(), 
                    main_title = "Total Variance Decomposition: Positive Mood, DN, and Positive Events", 
                    custom_filename = "{GRAPHICS_DIR}/S2_POS_PosEvnt_DN_combined.eps" %>% glue(), 
                    custom_title = "DN Variance Decomposition: Positive Mood, DN, and Positive Events", 
                    combined_plot_filename = "{GRAPHICS_DIR}/S2_POS_PosEvnt_full_decomp.eps" %>% glue())
